...
int decode_v4l2_frame(v4l2_dev_t *vd, v4l2_frame_buff_t *frame)
{
	...
	/*check if it is Leopard bayer cameras, it uses uvc v1.1*/
	if (my_device_list->list_devices[vd->this_device].vendor != 0x2a0b) {
	...
	case V4L2_PIX_FMT_NV42:
		...
		case V4L2_PIX_FMT_YUYV:
			/*check if it is Leopard bayer cameras*/
			if (my_device_list->list_devices[vd->this_device].vendor == 0x2a0b) {
				vd->isbayer = 1;
			}
			...
			if(vd->isbayer>0)
			{
				...
				/* for LI-IMX290, RAW12, right shift 4 bits, pixel order = GBRg*/
				else if (sensorid ==0x00d8) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				}

				/* for LI-IMX172, RAW12, right shift 4 bits, pixel order = GBRG*/
				else if (sensorid ==0x00e0) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				...
				/* for LI-IMX226, RAW12, right shift 4 bits, mono color*/
				else if (sensorid == 0x00e2) {
					mono2yu12(frame->raw_frame, frame->yuv_frame, width, height, 4);
				}

				/* for LI-OV13850, RAW12, right shift 4 bits, pixel order = BGGR*/
				else if (sensorid == 0x00e5) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
				...
				/* for LI-OV10640, RAW12, right shift 4 bits, pixel order = GBRG for 720p, BGGR for VGA*/
				else if (sensorid ==0x00e8) {	
					if (height == 720) {
						frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
						bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);//in out
						bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);//in out
						rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);//out in
					}
					else {
						frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
						bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);//in out
						bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 2);//in out
						rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);//out in
					}
				}

				/* for LI-IMX224, RAW12, right shift 4 bits, pixel order = GBRG*/
				else if (sensorid == 0x00e9) {
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				...
				/* for LI-AR1820, RAW10, right shift 2 bits, pixel order = GBRG*/
				else if (sensorid ==0x00ed) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
				...
				/* for LI-ICX285, RAW12, right shift 4 bits, pixel order = GBRG*/
				else if (sensorid ==0x00ee) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				...
				/* for LI-C570, RAW12, right shift 2 bits, pixel order = GBRG*/
				else if (sensorid ==0x00ed) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
				...
				/* for LI-AR0330 ICP1, YUV422*/
				else if (sensorid ==0x00f2) {		
					yuyv_to_yu12(frame->yuv_frame, frame->raw_frame, width, height);
				}

				/* for LI-MT9P031, RAW12, right shift 4 bits, pixel order = GRBG*/
				else if (sensorid ==0x00f4) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 1);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				...
				/* for LI-STEREO BAYER, RAW12, right shift 4 bits, pixel order = GBRG*/
				else if (sensorid ==0x00f5) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				...
				/* for LI-V034, RAW10, right shift 2 bits, pixel order = GBRG*/
				else if (sensorid ==0x00f7) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,2);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
				...
				/* for LI-M031, RAW12, right shift 4 bits, pixel order = GBRG*/
				else if (sensorid ==0x00f8) {		
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->tmp_buffer1, frame->tmp_buffer, width, height, 0);
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);
					...
					frame->tmp_buffer1 = calloc(width*height*3, sizeof(uint8_t));
					bayer16_convert_bayer8(frame->raw_frame, frame->tmp_buffer1, width, height,4);
					bayer_to_rgb24 (frame->raw_frame, frame->tmp_buffer, width, height, 0);//in out
					rgb24_to_yu12(frame->yuv_frame, frame->tmp_buffer, width, height);//out in
				}
			}
...
/*
 * For Leopard Bayer Camera RAW datatype conversion
 * Right shift certain bits for RAW datatypes(16 bits with MSB filled with 0)  
 */
void bayer16_convert_bayer8(uint8_t *inbuf, uint8_t *outbuf, int width, int height, int shift)
{
	int i = 0, j = 0;
